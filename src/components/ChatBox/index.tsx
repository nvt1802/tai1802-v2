...
const ChatBox = ({ user }: any) => {
  ...
  const handleOnSubmit = (event: any) => {
    ...
    if (messageRef !== null && messageInputRef !== null) {
      const messageValue: string = messageInputRef.current?.value || ""
      if (messageRef.current !== null && messageValue.length > 0) {
        createMessage(
          user?.displayName,
          user?.email,
          user?.photoURL,
  ...
  const handleScroll = () => {
    if (messageRef.current !== null && messageRef.current.scrollTop === 0) {
  ...
  return (
    <Fragment>
      <Tooltip
        title={
          <Fragment>{t("common:message_popup.title_toltip_message")}</Fragment>
        }
        aria-label="add"
      >
        <Fab
